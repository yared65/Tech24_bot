...
# ==========================================
# 5. DASHBOARD API SCRAPING & MUTATION (RE-ARCHITECTED FOR 419 BYPASS)
# ==========================================
async def scrape_website_cases():
    """Authenticates and pulls active ATM cases with rigid stateful cookie handling."""
    ...
    # Accurate stateful browser headers
    headers = {
    ...
    # Use a highly persistent client session to maintain the cookies in memory
    async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=30.0, verify=False) as client:
        try:
            ...
            # Inject headers into client instance
            client.headers.update({
                'X-XSRF-TOKEN': decoded_token,
                'X-CSRF-TOKEN': decoded_token,
            })
            
            # Explicitly set Cookie header to guarantee propagation
            ...
            # 3. Update cookies after successful login session initiation
            ...
            # Refresh client headers with the new active session cookies
            ...
            for item in cases_list:
                ...
                # Filter strictly for Adama (and bypass Adama District explicitly if needed)
